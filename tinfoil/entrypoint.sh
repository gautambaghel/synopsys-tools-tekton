...
if [[ $SITES_ID == "" ]]; then
  echo "SITES_ID has not been set. Aborting."
  exit 1
fi
...
BASE_URL='https://www.tinfoilsecurity.com/api/v1'
AUTH_HEADER="Authorization:Token token=${TOKEN}, access_key=${ACCESS_KEY}"

# [1] Start an API scan
START_SCAN_RESPONSE=$(curl -s -X POST "${BASE_URL}/sites/${SITES_ID}/scans" --header "${AUTH_HEADER}")
